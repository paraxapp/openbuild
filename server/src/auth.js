...
const { authPool, PRIMARY } = require('./db');
...
router.post('/register', async (req, res) => {
  try {
    ...
    res.json({ token: sign(row), user: row, authDb: PRIMARY });
  } catch (e) {
    console.error(e);
...
router.post('/login', async (req, res) => {
  try {
    ...
    res.json({ token: sign(user), user: { id: user.id, email: user.email }, authDb: PRIMARY });
  } catch (e) {
    console.error(e);
...
router.get('/me', requireAuth, (req, res) => {
  res.json({ user: { id: req.user.sub, email: req.user.email }, authDb: PRIMARY });
});
